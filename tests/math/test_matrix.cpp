...
namespace
{
    ...
    void test_element_access_is_independent_of_storage_order()
    {
        ...
        const matrix<float, 2, 3> row_major = matrix<float, 2, 3>::from_rows(math::vector<float, 3>{1.0f, 2.0f, 3.0f},
                                                                             math::vector<float, 3>{4.0f, 5.0f, 6.0f});

        const matrix<float, 2, 3, matrix_order::column_major> col_major =
            matrix<float, 2, 3, matrix_order::column_major>::from_rows(math::vector<float, 3>{1.0f, 2.0f, 3.0f},
                                                                       math::vector<float, 3>{4.0f, 5.0f, 6.0f});
    ...
    void test_from_rows_and_from_columns_are_transposes()
    {
        ...
        const auto by_rows =
            matrix<float, 2, 3>::from_rows(vector<float, 3>{1.0f, 2.0f, 3.0f}, vector<float, 3>{4.0f, 5.0f, 6.0f});

        const auto by_cols =
            matrix<float, 3, 2>::from_columns(vector<float, 3>{1.0f, 2.0f, 3.0f}, vector<float, 3>{4.0f, 5.0f, 6.0f});
    ...
    void test_identity_and_inverse()
    {
        ...
        const mat3f m = mat3f::from_rows(vec3f{2.0f, 0.0f, 1.0f}, vec3f{1.0f, 3.0f, 2.0f}, vec3f{1.0f, 1.0f, 2.0f});
        ...
        // than throwing.
        const mat3f singular =
            mat3f::from_rows(vec3f{1.0f, 2.0f, 3.0f}, vec3f{2.0f, 4.0f, 6.0f}, vec3f{1.0f, 1.0f, 1.0f});
    ...
    void test_transpose_and_trace()
    {
        ...
        const mat3f m = mat3f::from_rows(vec3f{1.0f, 2.0f, 3.0f}, vec3f{4.0f, 5.0f, 6.0f}, vec3f{7.0f, 8.0f, 9.0f});
