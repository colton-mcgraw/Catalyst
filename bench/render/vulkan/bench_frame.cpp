 ...
 * render pass that clears and stores it - separate submission and presentation overhead from the bandwidth a
 * full-target clear costs. The `quads` numbers should be read net of the clear-only frame. The suite closes with the
 * cost of recreating the swapchain, which is what a window resize pays.
 */
    ...
    void run_frame_suite(context &ctx, const options &opt)
    {
        ...
        print_frames("render pass, contents discarded (no clear, no store)",
                     run_frames(ctx, opt,
                                [](const command_list &cl, const texture &target)
                                {
                                    ...
                                    begin_render_pass(
                                        cl, {.color_attachments = std::span{&color, 1}, .debug_name = "discard"});
                                    end_render_pass(cl);
                                }));
        ...
        print_frames("render pass, cleared and stored",
                     run_frames(ctx, opt,
                                [](const command_list &cl, const texture &target)
                                {
                                    ...
                                    begin_render_pass(
                                        cl, {.color_attachments = std::span{&color, 1}, .debug_name = "clear"});
                                    end_render_pass(cl);
                                }));

        print_frames(
            "render pass with viewport and scissor set",
            run_frames(ctx, opt,
                       [extent](const command_list &cl, const texture &target)
                       {
                           const color_attachment color{
                               .target = target,
                               .load = load_op::clear,
                           ...
                           set_viewport(
                               cl, {0.0f, 0.0f, static_cast<float>(extent.width), static_cast<float>(extent.height)});
        ...
        // rect by the surface, so this measures the recreate rather than a genuine change of size.
        {
            ...
            const stats s = measure(iterations,
                                    [&]
                                    {
                                        const extent2d target =
                                            toggle ? extent : extent2d{extent.width / 2 + 1, extent.height / 2 + 1};
                                        toggle = !toggle;
                                        ctx.resize(target);
                                    });
            ctx.resize(extent);
