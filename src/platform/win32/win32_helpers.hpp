...
namespace catalyst::platform::detail::win32
{
     ...
     * @brief Declares this process per-monitor DPI aware, once. Idempotent, and safe to call from any entry point that
     * can be the first one an application reaches.
     * @details A process that never declares DPI awareness is treated as DPI-unaware, which is not a neutral default:
     * the DPI query APIs report a flat 96 regardless of the display, WM_DPICHANGED is never delivered, and the
     * compositor bitmap-stretches every window on any display scaled above 100%. Declaring awareness must happen before
     * any window is created and before any DPI is queried, so it is done here rather than in the window backend alone
     * -- an application that enumerates monitors before opening a window would otherwise cache DPI-unaware values.
     *
     * Per-monitor-v2 is preferred because it is the only context in which the non-client area scales with the window as
     * it moves between displays; the older contexts are tried in turn for older systems. If the application declared
     * awareness itself through an application manifest every call here fails harmlessly and the manifest's choice
